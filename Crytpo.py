...
def save_users(users):
    ...
    s3_client.put_object(
        Bucket=BUCKET_NAME, Key=USERS_CSV_S3_KEY, Body=csv_buffer.getvalue()
    )
...
def load_user_keys(user):
    try:
        private_key_obj = s3_client.get_object(
            Bucket=BUCKET_NAME, Key=f"{user}/private_key.pem"
        )
        public_key_obj = s3_client.get_object(
            Bucket=BUCKET_NAME, Key=f"{user}/public_key.pem"
        )

        private_key = serialization.load_pem_private_key(
            private_key_obj["Body"].read(), password=None
        )
    ...
    if choice == "Iniciar Sesión":
        ...
        if st.button("Autenticar Firma"):
            if document is not None and signature_file is not None and user:
                ...
                if public_key:
                    if verify_signature(
                        public_key, document_content, signature_content
                    ):
